...
本项目至今**唯一**一条 `measured` 一级数据 —— 即 [glossary.md](glossary.md) 定义的「**目标硬件实测**」:
在本项目候选机型上、由本项目亲自跑出来的读数。

> **不要把第三方读数算进这一级。** `data/models/qwen3.8-27b.js` 的 notes 里引了 vLLM recipe 在
> 1×/2× RTX 5090 上的几组引擎自报数字(KV 池 tokens、CUDA graph 在 util 预算之外等)。那些是
> **别人在非候选机型上**测的,只能当旁证:5090 不在 `data/instances.js` 里,也不是本项目跑的。
> 它们仍然有用(那条「CUDA graph 分配在 util 预算之外」的口径差异就是从那儿来的),但**不是
> `measured`**。

每条记录必须写全:机型、切分、**引擎版本(带 commit)与
