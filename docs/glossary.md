...
| `measured` | 目标硬件实测 | **每卡非权重非 KV 开销 6.98 GiB**，以及权重 38.24 GiB/卡、KV 池 81.79 GiB、128K 满长并发 55.15 路 —— 全部来自 2026-09-16 在 p5en 上对 GLM-5.3-Flash 起的那一次服务，见 [[measurements]] M-001。**只此一档**，页面上的数字并未因此改用实测值（见上一行）。还缺的：roofline 的效率比 `实测 ÷ 下界`（见 [[adr-0010]] §6）；KV 每 token 字节数（M-001 反解不出来 —— 混合池统一 page 把 recurrent state 与 20.75% padding 摊了进去，需换 `--max-num-seqs` 跑两点法） |
